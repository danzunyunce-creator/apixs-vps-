...
const ytApi = __importStar(require("./youtubeApi"));
...
class AutomationEngine {
    ...
    async syncLiveViewers() {
        ...
        for (let [id, desc] of this.sm.activeStreams) {
            ...
            if (!searchName)
                continue;
            try {
                dbLayer.db.get(`SELECT channel_id FROM youtube_channels WHERE channel_name = ? AND auth_type = 'OAuth' LIMIT 1`, [searchName], async (err, row) => {
                    if (row) {
                        await ytApi.execute(async (youtube) => {
                            ...
                            if (res.data.items && res.data.items.length > 0) {
                                ...
                                // Milestone Alert
                                if (concurrentViewers >= 100 && desc.lastMilestone !== 100) {
                                    desc.lastMilestone = 100;
                                    telegramService_1.telegramService.sendMessage(`🚀 <b>HOT STREAM!</b> Channel <i>${searchName}</i> menembus <b>100 penonton</b>!`).catch(() => { });
                                }
                            }
                        }, row.channel_id);
                    }
                ...
                console.warn(`[AutomationEngine] Failed to sync viewers for ${id}:`, e.message);
            }
        }
    }
    ...
    async sendHealthPulse() {
        ...
        telegramService_1.telegramService.sendMessage(message).catch(() => { });
    }
    async rotateStreamSEO() {
        for (let [id, desc] of this.sm.activeStreams) {
            this.runSeoUpdate(id, desc);
        }
    }
    async watchEmptyViewers() {
        if (!(await this.isRuleEnabled('Anti-Zonkz Stream')))
            return;
    ...
    async rotateScheduledStream(scheduleId) {
        dbLayer.db.get(`SELECT * FROM schedules WHERE id = ?`, [scheduleId], (err, schedule) => {
            if (err || !schedule)
                return;
            ...
            const streamUrl = config_1.default.RTMP_BASE_URL;
            const meta = {
    ...
    async checkScheduledAutoStarts() {
        try {
            dbLayer.db.all(`SELECT * FROM schedules WHERE status = 'SCHEDULED'`, [], async (err, rows) => {
                if (err)
                    return;
                const now = Date.now();
            ...
            // Check for Stop Time
            dbLayer.db.all(`SELECT * FROM schedules WHERE status = 'RUNNING' AND end_time IS NOT NULL`, [], (err, rows) => {
                if (err)
                    return;
                const now = Date.now();
                for (let schedule of rows) {
    ...
    trashCleanup() {
        ...
        try {
            ...
            if (fs_1.default.existsSync(config_1.default.UPLOADS_DIR)) {
                const files = fs_1.default.readdirSync(config_1.default.UPLOADS_DIR);
                const now = Date.now();
                files.forEach(f => {
                    const fullPath = path_1.default.join(config_1.default.UPLOADS_DIR, f);
    ...
    async isRuleEnabled(ruleName) {
        return new Promise((resolve) => {
            dbLayer.db.get(`SELECT enabled FROM automation_rules WHERE name = ?`, [ruleName], (err, row) => {
                if (err || !row)
                    resolve(false);
                else
                    resolve(row.enabled === 1 || row.enabled === true);
            });
        });
    }
    async onStreamStart(streamId, meta) {
    ...
    async runSeoUpdate(streamId, meta) {
        try {
            const searchName = meta.channel_name || meta.title || meta.name;
            dbLayer.db.get(`SELECT channel_id FROM youtube_channels WHERE channel_name = ? AND auth_type = 'OAuth' ORDER BY created_at DESC LIMIT 1`, [searchName], async (err, row) => {
                if (err)
                    return this.sm.emitLog(streamId, 'error', `[Auto-SEO Niche] Gagal melacak channel tujuan: ${err.message}`);
                ...
                try {
                    await ytApi.execute(async (youtube) => {
                        ...
                        if (broadcasts.data.items && broadcasts.data.items.length > 0) {
                            ...
                            const wordsForTags = safeCategory.split(' ').filter((w) => w.length > 3);
                            ...
                            telegramService_1.telegramService.sendMessage(`✅ <b>Automation SEO:</b> Judul & Deskripsi updated untuk channel <i>${meta.channel_name}</i>.`).catch(() => { });
                        }
                        else {
